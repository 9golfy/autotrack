...
import { buildStoragePublicUrl } from "@/services/supabase/storage-url";
import { analyzeCareImage, type CareImageAnalysis } from "@/services/analyze-care-image";
import { buildHealthReport, extractTimedVitalsSamples, type TimedVitalsSample } from "@/services/health-report";
import {
  ...
  evaluateSpo2,
  evaluateTemperature,
} from "@/services/health-thresholds";
import {
...
export async function POST(request: NextRequest) {
  ...
  // ── memberJoined event — บันทึก profile ของสมาชิกใหม่ลง DB ──────────────
  const memberJoinedEvents = (payload.events ?? []).filter(
    (event) => event.type === "memberJoined",
  );
  ...
  for (const event of memberJoinedEvents) {
    ...
    const joinedMembers = (event as unknown as { joined?: { members?: { type: string; userId: string }[] } }).joined?.members ?? [];

    for (const member of joinedMembers) {
      if (member.type !== "user" || !member.userId) continue;

      try {
        // ดึง profile ของ member ใหม่
        const profile = await fetchLineApiJson<{
          displayName?: string;
          pictureUrl?: string;
          statusMessage?: string;
        }>(`/v2/bot/group/${groupId}/member/${member.userId}`);

        // บันทึกเป็น "join" message เพื่อให้ member list เห็น
        await prisma.message.upsert({
          where: { messageId: `join-${groupId}-${member.userId}` },
          create: {
            messageId: `join-${groupId}-${member.userId}`,
        ...
        console.info("Saved memberJoined", { groupId, userId: member.userId, displayName: profile.displayName });
      } catch (err) {
        console.warn("Failed to save memberJoined", { groupId, userId: member.userId, err });
      }
    }
  }
